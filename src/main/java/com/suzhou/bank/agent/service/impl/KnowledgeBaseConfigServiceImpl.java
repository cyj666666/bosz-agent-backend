...
public class KnowledgeBaseConfigServiceImpl implements IknowledgeBaseConfigService {
    ...
    /*
     * 2026-09-24 移除：原「统一追加『不要输出标题』约束」（常量 NO_TITLE_RULE + 方法 withNoTitleRule）。
     *
     * 它曾在两处生效：sendAnswer（知识配置页预览）与 handleMorePromptContent（报告链路主路径）。
     * 移除依据（曹哥 2026-09-24 口径「没啥用，不要了」）：
     *   · 标题本就由**结构字段**渲染（章节标题取 catalogName、块标题取 blockName），不依赖模型写；
     *   · 84 条知识库配置里多数自带「禁止输出 Markdown 标题」这类约束，本常量只是给没写的那些兜底，
     *     上游生效时它属于重复约束；
     *   · 前端同源的渲染层去重（useReportInstance 的 stripDuplicatedTitle）也已一并移除。
     * ⚠️ 若将来正文重现「标题重复一行」，优先在**知识库配置的 contentdesc** 里加约束，
     *     而不是把本方法加回来。
     */
    ...
    private List<String> getKnowledgeIdListByRoleId() {
        ...
        //    （按 `sys_role.menu_permissions` 含裸 "*" 判定，走 agentRoleMapper#countFullMenuRoles）。
        //    客户最终确定的角色模型里**管理员并不持有 "*"**（管理员默认只看报告管理/智策引擎/系统管理）
        //    ⇒ 该旁路既失效、又会让"数据授权页里配的东西不生效"，故整体移除。
        //    现回归**纯数据表驱动**：知识可见性完全由 sys_role_knowledge 决定。
        //    ⚠️ 因此 admin 的 sys_role_knowledge 必须有数据，否则知识配置页会空白 —— 见增量 DML。

        // 按 sys_role_knowledge 的授权过滤（fail-closed：查不到 ⇒ 空列表 ⇒ 调用方 return new ListResult<>(0, 0)）
        List<String> knowledgeIdList = null;
        String role = apiContextModel.getRole();
        if (StringUtils.isNotEmpty(role)) {
            List<String> roleIdList = JSON.parseArray(role, String.class);
