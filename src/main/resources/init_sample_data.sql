...
-- ==================== 3. 知识库规则 ====================
INSERT INTO knowledge_rule (rule_code, rule_name, rule_type, description, enabled, created_at) VALUES
('R001', '实控人/重要股东/法人代表变更', 'BOOLEAN', '实际控制人变更、受益人变更、重要股东名单发生变更、法人代表/董监高变更。智能体基于大模型通识做深度推理。', 1, NOW()),
('R002', '股权冻结及累计质押比例过高', 'THRESHOLD', '新增股权冻结、新增股权出质且累计质押比例过高、实控新增股权出质且累计质押比例过高。', 1, NOW()),
('R003', '工商经营异常', 'BOOLEAN', '企业被列入工商经营异常名录。', 1, NOW()),
('R004', '重大负面舆情', 'BOOLEAN', '证监立案调查等重大负面舆情。待行内分析后确定具体判定条件。', 1, NOW()),
('R005', '资产负债率异常攀升', 'THRESHOLD', '资产负债率较去年同期显著攀升（国企↑≥5%，通用↑≥10%）。智能体对财务异动进行归因分析。', 1, NOW()),
('R006', '经营现金流持续为负且流动性快速消耗', 'COMPOSITE', '经营现金流持续(近4季≥3季/连续2年)为负且货币资金下降≥30%。智能体对财务异动进行归因分析。', 1, NOW()),
('R007', '经营性现金流实质性落后于收入', 'THRESHOLD', '净现比同比下降≥10%，经营现金流实质性落后于收入增长。智能体对财务异动进行归因分析。', 1, NOW()),
('R008', '收入增速异常偏离经营基本面', 'COMPOSITE', '营收同比增速连续2年高于应收账款增速+30%、存货增速+30%、现金流增速+30%。智能体对财务异动进行归因分析。', 1, NOW()),
('R009', '应收账款回收周期延长', 'THRESHOLD', '应收账款周转天数较去年同期延长超过60天或DSO翻倍。智能体对财务异动进行归因分析。', 1, NOW()),
('R010', '他行授信压缩/抽贷', 'THRESHOLD', '全部他行授信总额大幅下降≥30%，疑似被其他金融机构压缩授信或抽贷。', 1, NOW()),
('R011', '新增非银融资及征信查询异常', 'THRESHOLD', '非银融资余额增长≥30%、新增非银机构≥2家、近3月征信查询≥6次，融资渠道转向高成本非银机构。', 1, NOW()),
('R012', '对外担保大幅增加', 'THRESHOLD', '对外担保余额较上期增长≥50%，或对外担保余额/净资产>50%（国企>60%）。', 1, NOW()),
('R013', '征信新增逾期/垫款/代偿记录', 'BOOLEAN', '征信出现新增逾期>30天、多笔逾期、垫款或代偿记录、资产管理公司介入处置。', 1, NOW()),
('R014', '债务重组/展期及分类劣化', 'BOOLEAN', '出现债务重组或展期记录、贷款五级分类劣化为关注及以下。', 1, NOW()),
('R015', '信贷资金疑似回流或用途异常', 'BOOLEAN', '信贷资金疑似回流、资金用途疑似异常、受托支付资金疑似归集。以行内图谱判定结果为准。', 1, NOW()),
('R016', '上下游客户异常', 'BOOLEAN', '上下游客户出现失信被执行人或被执行人、结算与授信申报上下游客户不匹配（≥3家不一致）。', 1, NOW()),
('R017', '纳税申报异常', 'THRESHOLD', '纳税申报销售额同比大幅下降（小微↓≥30%，通用↓≥50%），企业实际经营规模收缩。', 1, NOW()),
('R018', '经营运营数据异常（社保、水电气、海关）', 'COMPOSITE', '通用企业：用电↓≥15%且社保↓≥15%且纳税↓≥20%；纺织化工：用水骤降≥50%；外贸企业：海关出口骤降≥50%。交叉验证。', 1, NOW()),
('R019', '政府管理部门负面信息', 'BOOLEAN', '新增行政处罚且处罚金额>50万、环保评级不良/严重不良、严重违法记录。', 1, NOW()),
('R020', '新增作为被告的关键法诉记录', 'THRESHOLD', '任意一种满足：涉案金额>500万或>20%净资产、涉刑/破产。智能体基于大模型通识做深度推理。', 1, NOW()),
('R021', '代发工资及用工异常', 'THRESHOLD', '连续2个月代发人数环比减少>30%，企业用工规模显著收缩。', 1, NOW()),
('R022', '疑似借名贷款', 'BOOLEAN', '关联方交易及资金流向疑似存在借名贷款行为。以行内图谱判定结果为准。', 1, NOW()),
('R023', '疑似担保圈链', 'BOOLEAN', '企业对外担保关系复杂，疑似构成担保圈链风险。以行内图谱判定结果为准。', 1, NOW()),
('R024', '疑似与黑名单客户构成关联关系', 'BOOLEAN', '企业关联方中存在黑名单客户，疑似存在风险传导。以行内图谱判定结果为准。', 1, NOW()),
('R025', '疑似未纳入集团授信', 'BOOLEAN', '关联企业疑似应纳入集团授信管理但尚未纳入。以行内图谱判定结果为准。', 1, NOW());
