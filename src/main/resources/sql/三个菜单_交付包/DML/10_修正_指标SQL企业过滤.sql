...
--       `f.REPORTNO = :reportNo` / `f.CUSTOMERNAME = :entName`。
--       （2026-09-17 口径调整：customerId / guarantorId 已废弃，不再作为过滤条件；
--         担保人维度只用 guarantorName。故本文件只补 reportNo + entName 两个条件。）
--       这两个命名参数在 `paramData` 里**早已声明**（默认值 RPT-202603-001 /
...
SELECT paramno,
       ...
       CASE WHEN script LIKE '%:guarantorName%' THEN 1 ELSE 0 END AS has_guarantor_filter,
