-- =====================================================================
-- 苏州银行「贷后管理智能体」— 三个菜单涉及的全部建表 DDL
--
-- 覆盖菜单：
--   1) 指标配置   /agent/index-config
--   2) 知识配置   /agent/knowledge-config
--   3) 智策引擎   /agent/rule
--
-- 表数：44 张（13 + 11 + 2 + 13 + 5）
--   A 指标配置 13 / B 知识配置 11 / C 智策引擎 2 / D 三菜单共用支撑 13
--   E 覆盖率补漏 5 —— 代码真实查询、但**不在实体 @TableName 映射里**，早先按实体抽取时会漏掉：
...
--   ② 必须先 SET search_path = <schema>, public;（本脚本不含，按目标库自行指定）；
...
--     （说明"填值规则改本地兜底"与"数据权限规则刻意未复刻"），**运行期不查**，无需建表。
--   · 4 张表在源脚本里**没有表级注释**（sys_data_source / sys_category /
--     large_model_config / call_llm_record），本脚本与源一致，未擅自补写。
-- =====================================================================
