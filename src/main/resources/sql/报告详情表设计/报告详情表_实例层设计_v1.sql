...
--    · ⚠️ 2026-09-17 **移除**了「agentCode 在单份报告内唯一（唯一键 reportNo + agentCode）」：
--      同一 agentCode 在不同章节是**合法复用**的（六章=借款人口径 / 十二（二）=担保人口径），
--      入参不同、结果不同，必须各出一条 ⇒ 只保留 reportNo+blockCode；详见下方 app_report_ai_risk 段
--    · 跳转锚点单向
-- 日期     ：2026-09-10
-- =====================================================================
